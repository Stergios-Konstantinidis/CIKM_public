...
def load_paddle_records() -> list[dict]:
    BASE = Path(__file__).resolve().parent.parent.parent
    RESULTS = BASE / "results"
    GT_PATH = BASE / "data" / "evaluation_dataset" / "groundtruth.json"
    BASELINE = RESULTS / "baselines/baseline_paddle.json"
    ENGINE = "paddle"
    ...
    for r in valid:
        r["engine"] = ENGINE
    print(f"  [paddle] {len(valid)} valid baseline records")
    meta_lookup = load_metadata_lookup(GT_PATH)
    conf_lookup = load_confidence_lookup(ENGINE, RESULTS)
    print(f"  [paddle] confidence lookup: {len(conf_lookup)} entries")
...
def main():
    print("Loading data...")
    records = load_paddle_records()
    target_file = "corrections/paddle/paddle_Full_Advanced_5_google__gemini-3-flash-preview.json"
    ...
    for pp in range(0, 16):
        min_delta = pp / 100.0
        out_path = f"results/figures/routing_frontier/routing_frontier_paddle_{pp}pp.png"
        if Path(out_path).exists():
            print(f"\nSkipping δ > {pp}pp — {out_path} already exists")
            continue
        ...
        fig.suptitle(f'Routing Frontier (PaddleOCR) — δ > {pp}pp', fontsize=16, y=1.01)
        plt.tight_layout()
