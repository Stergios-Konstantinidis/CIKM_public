"""
Generate the routing frontier graph evaluating perfect human corrections.

Four curves per panel:
  (a) Oracle — sort perfectly by the actual raw error
  (b) Orthographic Correction — non-LLM spellcheck baseline (flat line)
  (c) ConfBERT — confidence-aware BERT model
  (d) Our Approach — LogReg classifier based on predicted error + base features

X-axis: % of Documents Corrected
Secondary x-axis: cumulative token cost

Output: results/human_corrected/human_routing_graph.png
"""
import sys
import warnings
import numpy as np
...
OUTPUT_DIR = RESULTS / "human_corrected"


def train_delta_regressions(X, records, corrections):
    # Since ground truth is perfect, corrected WER/CER is 0.0
    # The delta (improvement) is exactly the raw WER/CER
    delta_wer = np.array([float(r["wer"]) for r in records], dtype=np.float32)
    delta_cer = np.array([float(r["cer"]) for r in records], dtype=np.float32)
...
def train_our_classifier(X_stacked, delta_target, min_delta):
    ...
    # If all documents require correction (e.g. they all have delta > 0),
    # LogisticRegression will fail. We handle this edge case:
    if len(np.unique(y)) < 2:
        print("  Warning: Classifier only found one class. Using regression scores instead.")
        # If we can't train a classifier, we can just return the stacked delta feature directly
        # In X_stacked, index 2 is pred_ridge_dc and index 4 is pred_mlp_dc
        return X_stacked[:, 2] # Use predicted Ridge delta cer
...
def plot_subplot(ax, display_metric, records, corrections,
    ...
    # For human corrections, full == capped (all deltas >= 0), no faded line needed
    ...
    confbert_pct, confbert_full, confbert_capped = compute_routing_curves(confbert_idx, base_vals, corr_vals, N, deltas, 0.0)
    ...
    our_pct, our_full, our_capped = compute_routing_curves(our_idx, base_vals, corr_vals, N, deltas, 0.0)
    ...
    ax.set_xlabel('% of Documents Corrected (Human Effort)', fontsize=11)
...
def main():
    ...
    # Simulate human correction: ground truth is perfect (0.0 WER, 0.0 CER)
    corrections = {
        r["filename"]: {"wer": 0.0, "cer": 0.0}
        for r in records
    }
    ...
    # Use median delta so classifier has meaningful class separation
    # (with min_delta=0.0 all docs are positive → single class → fallback)
    min_delta = float(np.median(delta_cer_gt))
    print(f"Training our classifier (LogReg on δ-CER > {min_delta:.4f})...")
    our_probas = train_our_classifier(X_stacked, delta_cer_gt, min_delta)

    # ── ConfBERT ──
    print("Training ConfBERT (10-fold CV)...")
    # For human corrected scenario we should cache separately, but train_confbert_router uses corrections
    # We will compute it without saving over the LLM cache.
    confbert_cache = OUTPUT_DIR / "confbert_probas_human.npy"
    ...
    plt.tight_layout()
    out_path = OUTPUT_DIR / "human_routing_graph.png"
