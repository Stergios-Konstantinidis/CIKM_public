"""
experiment_svm_classifier.py
=============================
Binary classifier (Support Vector Machine) for selective OCR correction routing.
...
Outputs
-------
  results/svm_classifier_results.json
"""
...
def compute_feature_importances(X, y, feat_names, top_k=20):
    # RBF SVM does not inherently provide feature importances
    return []
...
def main():
    print("\n" + "=" * 65)
    print("SVM Classifier — Selective OCR Correction Routing (LOO)")
    print("=" * 65)
    ...
    # ── save ──────────────────────────────────────────────────────────────────
    out = {
        "experiment":    "svm_classifier_loo",
        "engine":        ENGINE,
        "n_records":     len(records),
        "n_features":    X.shape[1],
        "feature_names": feat_names,
        "svm_params":    SVC_PARAMS,
        "per_threshold": per_threshold_results,
        "per_record":    per_record_rows,
    }
    out_path = RESULTS / "ml_models/svm_classifier_results.json"
